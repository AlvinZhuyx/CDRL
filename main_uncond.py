...
MCMC_with_p_ema = False # when update samples propose by p using ema of pi or using current value of pi (only used when updating p)
...
def gen_samples(bs, p, pi, xt=None, device=None):
    ...
    for i in reversed(range(1, n_interval + 1)):
        ...
        with torch.no_grad():
            if p_with_noise:
                xtminus1_neg0 = p(xt, logsnr_t) + torch.sqrt(beta_t_tilt * as_t_mul).reshape((len(xt), 1, 1, 1)) * torch.randn_like(xt)
            else:
        ...
        if i == 1:
            # As described in appendix A.2, we do one more "denoising step" according to
            # Tweedie's formula at the final stage 
            xt_in = xt.detach().clone()
            xt_in.requires_grad = True
            en = pi(xt_in * torch.sqrt(as_t_mul).reshape((len(xt), 1, 1, 1)), logsnr_s)
            en = en / sz_square
...
def Langevin(pi, x, xt, logsnr_t, logsnr_tminus1, is_final, n_step=None, with_noise=True):
    ...
    for i in range(n_step):
        ...
        # as described in Appendix A.2, we parameterize the energy as the network_output / (sz ^ 2)
        # thus, the coefficients before grad term becomes 1/2 (step size get cancelled out) 
        # this parameterization helps the training
...
def train(args):
    ...
    if load_dir is not None and os.path.exists(os.path.join(load_dir, 'real_m.npy') and os.path.join(load_dir, 'real_s.npy')):
        print('Load from pre-calculated files')
        real_m = np.load(os.path.join(load_dir, 'real_m.npy'))
        real_s = np.load(os.path.join(load_dir, 'real_s.npy'))
    ...
    if dataset_type == 'cifar10':
        pi = EBM(add_q=add_q, temb_dim=pi_dim, n_blocks=pi_n_blocks, use_sn=with_sn, act_fn=ebm_act)
        p = Unet(tanh_out=False, residual=True, dim=p_dim)
    else:
        raise NotImplementedError
    ...
    if mode == 'fid':
        ...
        if device == torch.device('cuda', 0):
            ...
            save_name = '{}/fid_samples.png'.format(os.path.join(log_path, timestamp))
        with pi_ema.average_parameters():
            with p_ema.average_parameters():
                out_fid, fid_samples = calculate_fid(n_fid_samples_full, pi=pi, p=p, real_m=real_m, real_s=real_s, save_name=save_name, return_samples=True, device=device)
                fid_samples = fid_samples.detach().cpu()
                fid_array = fid_samples.numpy() * 2.0 - 1.0

        for i in range(len(fid_samples)):
            torchvision.utils.save_image(fid_samples[i], os.path.join(gen_sample_dir, '{}.png'.format(len(fid_samples) * args.local_rank + i)), normalize=True)
        print("Testing Fid Score {:.2f}".format(out_fid))
        return
    ...
    for counter in range(start_iter, iterations):
        ...
        pi.train()
        pos_energy =  pi(xtminus1 *  torch.sqrt(as_t_mul.reshape((len(x), 1, 1, 1))), logsnr_tminus1) 
        pos_loss = -(pos_energy * pi_loss_weight).mean()
        pos_loss.backward()
        p.train()
        if p_with_noise:
            xtminus1_neg0 = p(xt, logsnr_t) + torch.sqrt(beta_t_tilt * as_t_mul).reshape((len(x), 1, 1, 1)) * torch.randn_like(xt)
        else:
            xtminus1_neg0 = p(xt, logsnr_t)
        xtminus1_negk = xtminus1_neg0.detach().clone()
        xtminus1_negk.requires_grad = True
        ...
        neg_energy = pi(xtminus1_negk * torch.sqrt(as_t_mul.reshape((len(x), 1, 1, 1))), logsnr_tminus1)
        ...
        if counter % plot_iter == 0:
            pi.eval()
            p.eval()
            samples = gen_samples(bs=49, p=p, pi=pi, device=device)
            with pi_ema.average_parameters():
                with p_ema.average_parameters():
                    ema_samples = gen_samples(bs=49, p=p, pi=pi, device=device)

            if device == torch.device('cuda', 0):
                save_images = samples.detach().cpu()
                torchvision.utils.save_image(torch.clamp(save_images, min=-1.0, max=1.0), '{}/{}_p_samples.png'.format(img_dir, counter), normalize=True, nrow=7)
                save_images = ema_samples.detach().cpu()
                torchvision.utils.save_image(torch.clamp(save_images, min=-1.0, max=1.0), '{}/{}_ema_samples.png'.format(img_dir, counter), normalize=True, nrow=7)
                save_images = xtminus1[:49].detach().cpu()
                torchvision.utils.save_image(torch.clamp(save_images, min=-1.0, max=1.0), '{}/{}_z_tminus1.png'.format(img_dir, counter), normalize=True, nrow=7)
                save_images = xtminus1_neg0[:49].detach().cpu()
                torchvision.utils.save_image(torch.clamp(save_images, min=-1.0, max=1.0), '{}/{}_z_tminus1_neg0_sample.png'.format(img_dir, counter), normalize=True, nrow=7)
                save_images = xtminus1_negk[:49].detach().cpu()
                torchvision.utils.save_image(torch.clamp(save_images, min=-1.0, max=1.0), '{}/{}_z_tminus1_negk_sample.png'.format(img_dir, counter), normalize=True, nrow=7)
